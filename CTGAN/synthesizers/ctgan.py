...
class Discriminator(Module):
    ...
    def __init__(self, input_dim, discriminator_dim, pac=10):
        ...
        self.pacdim = dim
        seq = []
        for item in list(discriminator_dim):  # discriminator_dim=(256,256)
    ...
    def fit(self, train_data, discrete_columns=tuple(), epochs=None):
        ...
        for i in range(epochs):
            ...
            for id_ in range(steps_per_epoch):
                ...
                ##################################训练辨别器##############################################
                #从正态分布中随机
                fakez = torch.normal(mean=mean, std=std)   # 产生[500,128]的符合高斯分布得到随机矩阵

                condvec = self._data_sampler.sample_condvec(self._batch_size)   #如果没有离散列则返回None  条件生成器为不平衡列做准备的
                # if condvec is None:   # 没有离散列的情况
                ...
                fakez = torch.cat([fakez, c1], dim=1)       # 合并 形成[500,130]的噪声矩阵
                ...
                fake = self._generator(fakez)          # 噪声输入到生成器中生成符合要求的数据 fake=[m,]
                fakeact = self._apply_activate(fake)   # 对生成的数据进行激活操作  fakeact=[m,]
            ...
            print("Epoch {}  Loss G:{:.3f}  Loss D:{:.3f} ".format(i+1, total_loss_g, total_loss_d))
